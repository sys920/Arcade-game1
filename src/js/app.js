...
var level = 1;
var highestLevel = 0; //Increase speed when player is level up 

var increaseSpeed = 1; // Enemies our player must avoid
...
function checkCollisions(enemyX, enemyY) {
  if (Math.abs(enemyX - player.x) < 60 && Math.abs(enemyY - player.y) < 40) {
    ...
    if (level >= highestLevel) {
      highestLevel = level; // Alert for the level
    ...
    player.y = 400;
    level = 1;
    increaseSpeed = 1; // After collision, Reset Enemies's speed 
...
Player.prototype.handleInput = function (key) {
  if (key === "left" && this.x > 0) {
    this.x = this.x - 100; // check collision between stone and player this. is player

    collisitonWithStone = checkblock(this.x, this.y); // If checkblock function returns true, Do not move player this direction
  ...
  if (this.y === -10) {
    ...
    player.y = 400;
    level++;
    increaseSpeed++; // Alert for passing the level

    swal("Good job! You are level ".concat(level)); // Icrease allEnemiies' speed 
    ...
    stone.y = Math.floor(Math.random() * 3) * 100 + 40; // changing current level
...
var allEnemies = [new Enemy(-10, 65, 4), new Enemy(-40, 150, 2), new Enemy(0, 230, 5)];
var player = new Player(200, 400); // This listens for key presses and sends the keys to your
...
Stone.prototype.render = function () {
  ctx.drawImage(Resources.get(this.sprite), this.x, this.y); // put text on the game board

  ctx.font = "25px Arial";
  ctx.fillStyle = "white";
  ctx.fillText("HIGHEST LEVEL! : ".concat(highestLevel, " "), 10, 80);
  ctx.fillText("LEVEL : ".concat(level, " "), 10, 110);
};

var stone = new Stone(); // check collision between player and stone object
